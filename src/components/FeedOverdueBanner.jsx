...
export default function FeedOverdueBanner({ events, className = '' }) {
  ...
  return (
    <div className={`rounded-xl border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950 px-4 py-3 flex items-center gap-2 ${className}`}>
      <span className="text-xl shrink-0">🍼</span>
      <div className="min-w-0">
        <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">
          {elapsed} since last {status.label ?? 'feed'}
        </p>
        <p className="text-xs text-amber-600 dark:text-amber-400">
          at {new Date(status.lastFeed.timestamp_start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
        </p>
      </div>
    </div>
  )
